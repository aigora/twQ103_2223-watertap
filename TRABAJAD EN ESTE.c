	...
	do{
		...
		switch (a) {
			...
			case(4): {
				...
				printf ("============COMPARACIÓN=============\n"); 
				printf("Has seleccionado la opción de comparación de datos \n "); 
				float a, ph; 
				int fuente1, fuente2, opcion, n;
				float comparacion;  
		...
		// menu de opciones para seleccionar que tipo de dato queremos comparar
				do{
					...
					if (opcion<1 && opcion>4) {
						printf(" ERROR. Esa opción no esta disponible, pruebe otra vez por favor \n"); 
					}
			...
			switch (opcion)  {
				case (1): 
						...
						if (n==1){
							...
							printf("El menor valor de ph es: %d", comparacion);
						}
						else if (n==3){
...
float fcomparacionmayor (struct Tfuente fuente[]) {
		...
		//	}
		//	else if (opcion==3) {
				if(fuente[i].turbidez>mayor){
				mayor=fuente[i].turbidez;
				}
		//	}
		//	else if (opcion==4){
