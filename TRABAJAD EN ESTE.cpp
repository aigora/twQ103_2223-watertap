...
void fmediaph21(struct Tfuente[],int ndatos);
void fmediacond1(struct Tfuente[],int ndatos);
void fmediacond2(struct Tfuente[],int ndatos);
void fmediaturb1(struct Tfuente[],int ndatos);
void fmediaturb2(struct Tfuente[], int ndatos);
void fmediacoliformes1(struct Tfuente[],int ndatos);
	...
	do{
		...
		switch (a) {
			...
			case(4): {
				...
				printf("Ha seleccionado la opcion de las estadísticas.\n");
				printf("=========* MENU ESTADÍSTICAS *=========\n");
				FILE *fsalida;
				...
				do { 
					...
					switch (opcion) {
						...
						case 2:
							...
							//coliformes 1
							if(md == 1 && sobremd == 4) {
								fmediacoliformes1(fuente, ndatos);
							}
							//coliformes 2
							if(md == 2 && sobremd == 4) {
						...
						case 3:
							...
							for(i = 0; i < ndatos; i++){
    							for(j = 0; j < ndatos; j++){
       								 if(fuente[j].ph > fuente[j + 1].ph){
           								aux = fuente[j].ph;
           								fuente[j].ph = fuente[j + 1].ph;
            							fuente[j + 1].ph = aux;
       								}//fin if
    							}//fin for
  							}//fin for
  							if(ndatos%2==0){
  								printf("La mediana es %f",fuente[ndatos/2].ph);
							  }
							if(ndatos%2!=0){
				...
				switch(dato1) {
					case 1: {
						...
						if(dato2==1) {
							...
							r=sqrt(b*beta);
							if(r>0.5) {
								printf("La correlación es significativa puesto que es %f", r);
							} else {
								printf("La correlación no es significativa, puesto que es %f", r);
							}
...
void fmediacoliformes2(struct Tfuente fuente[],int ndatos){
	int i,non;
	for (i=0; i<ndatos; i++) {
		printf("Introduzca el número de la fuente cuyo pH desea calcular la media (minimo 2), si no quiere introducir mas pulse el 0. \n");
		scanf("%d", &non);
		if(non==0) {
			break;
		} else {
